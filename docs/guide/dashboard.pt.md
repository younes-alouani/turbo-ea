...
Administradores (qualquer função com `admin.users`) veem um widget **Diretório de partes interessadas** na parte inferior da aba Administração. Ele lista cada tipo de cartão com pelo menos uma parte interessada, junto com o número de titulares distintos. Expanda um tipo de cartão para ver suas funções e, dentro de cada função, os usuários com o número de cartões que cobrem. É a resposta em nível de organização à pergunta «quem é responsável por quê?», em uma única tela e com um clique por tipo de cartão.

Os chips do widget são **sensíveis ao hover**: pause o cursor sobre qualquer chip de usuário no diretório — ou sobre um nome de parte interessada na aba Partes interessadas de um cartão ou sobre um proprietário de risco no Registro de riscos / na página de detalhe de risco — e um pequeno popover abre com o portfólio completo daquela pessoa agrupado por função. Clique em qualquer cartão no popover para ir diretamente até ele. O popover só busca uma vez por usuário por sessão.
